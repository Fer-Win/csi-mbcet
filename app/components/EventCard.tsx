...
const EventCard:React.FC<EventCardProps> = ({EventName,Date,EventType,Tags,ImageURL,registeredNo}) => {
  ...
  return (
    <div className='hover:scale-110 transition-all ease-in duration-150  p-2 overflow-hidden flex flex-col w-1/5 rounded-md text-2xl font-bold gap-5 h-fit min-h-max border-white border border-opacity-25 hover:bg-white hover:bg-opacity-5'>
        <div className=' overflow-hidden  bg-center bg-cover relative'>
           
            <div className='text-sm font-normal l mb-1 flex flex-col justify-center items-center px-2 text-center rounded-md py-1 right-3 top-3 bg-white leading-6 text-black font-bold z-10 absolute'>{Date}</div>
